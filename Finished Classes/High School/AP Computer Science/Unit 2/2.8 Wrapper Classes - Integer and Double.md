...
The maximum amount of bits per integer in `Java` (along with `C` and `C++`!) is 32 bits. If each bit represents a 0 or 1 in binary, the maximum number an integer can hold is ${2^{31}}$ or `2147483648` (one bit is used to hold the sign).
